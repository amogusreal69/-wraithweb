...
(function($) {
    ...
    // rangeslider end
     ...
     $(function(){
        ...
        //         $('.server_location').css('display','none');
        //     }else{
        //         $('.opt-2').css('display','none');
        ...
        $(document).ready(function() {
            // Initially hide VPS options
            $('.opt-2').css('display', 'none');
        
            $("#sw-1").on("click", function() {
                if ($(this).is(":checked")) {
                    // Show VPS Plan options
                    $('.opt-1').css('display', 'none');
                    $('.opt-2').css('display', 'inline-block');
                    
                    // Update server location dropdown for VPS Plan
                    updateServerLocations('vps');
                } else {
                    // Show Game Plan options
                    $('.opt-2').css('display', 'none');
                    $('.opt-1').css('display', 'inline-block');
                    
                    // Update server location dropdown for Game Plan
                    updateServerLocations('game');
                }
            });
        
            function updateServerLocations(planType) {
                var $serverLocationDropdown = $('.server_location');
        
                // Clear current options
                $serverLocationDropdown.empty();
        
                if (planType === 'game') {
                    // Add Game Plan server location
                    $serverLocationDropdown.append('<option class="opt-1" value="india" data-src="images/flags/india.png">India</option>');
                } else if (planType === 'vps') {
                    // Add VPS Plan server locations
                    $serverLocationDropdown.append('<option class="opt-2" value="india" data-src="images/flags/india.png">India</option>');
                    $serverLocationDropdown.append('<option class="opt-2" value="germany" data-src="images/flags/germany.png">Germany</option>');
                    $serverLocationDropdown.append('<option class="opt-2" value="singapore" data-src="images/flags/singapore.png">Singapore</option>');
                }
            }
        
            // Initialize server location dropdown with default plan
            updateServerLocations('game');
        });
